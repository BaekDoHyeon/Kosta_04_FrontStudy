...
public class MyBatisTestApp05 {
	...
	public static void main(String[] args) throws Exception{
		...
		System.out.println(":: 2. addUser(INSERT)  ? " + session.insert("UserMapper05.addUser",user));//1
		session.commit();
		System.out.println("\n");
		
		//3. User05.uadateUser Test  :: users table age/redDate 입력값 확인할것 : OK
		// :: 주몽 ==> 온달 수정
		user.setUserName("온달"); //user05 를 가진 주몽 --> 온달
		System.out.println(":: 3. update(UPDATE)  ? " + session.update("UserMapper05.updateUser",user));//1
		session.commit(); //MyBatis 만 돌릴때는 Commit 을 해주어야 한다. (DML일때..)
		
		//4. user05 아이디를 가진 사람을 삭제하는 단위테스트를 작성
		System.out.println(":: 4. delete(DELETE)  ? " + session.delete("UserMapper05.removeUser", user.getUserId()));//1
		session.commit();
