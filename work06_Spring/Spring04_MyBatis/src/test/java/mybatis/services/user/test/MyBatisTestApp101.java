...
public class MyBatisTestApp101 {
	...
	public static void main(String[] args) throws Exception{
		...
		System.out.println(":: 1. addUser(INSERT)  ? " + session.insert("UserMapper10.addUser",user)); //1
		session.commit();
		...
		// :: 주몽 ==> 온달 수정
		user.setUserName("온달");
		System.out.println(":: 3. update(UPDATE)  ? " + session.update("UserMapper10.updateUser",user));//1
		session.commit();
		...
		System.out.println(":: 5. Use10.removeUser (DELETE)  ? " +session.delete("UserMapper10.removeUser",user.getUserId()) );//1
		session.commit();
