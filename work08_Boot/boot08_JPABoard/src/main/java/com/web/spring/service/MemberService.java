...
public class MemberService {
	...
	public Member signIn(String id, String pwd) {
		Member rMember = memberRepository.duplicateCheck(id);
		if(rMember == null) throw new MemberAuthenticationException("아이디를 다시 확인하세요.","Wrong ID~~!!");
		if(rMember.getPwd().equals(pwd)) throw new MemberAuthenticationException("비밀번호를 다시 확인하세요.", "Wrong PWD~~!!");
		return rMember;
	}
}
